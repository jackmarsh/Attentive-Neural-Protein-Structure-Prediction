class PNeRF():
    ...
    # TODO: Engh & Huber ideal values
    
    def __init__(self, rs, thetas):
        self.r = np.array([1.45801, 1.52326, 1.32868], dtype='float32')
        self.theta  = np.array([  2.124,   1.941,   2.028], dtype='float32')
        self.num_dihedrals = 3
        self.num_dimensions = 3
        
        self.rs = rs
        self.thetas = thetas
#         self.rs = np.array([[1.45801, 1.52326, 1.32868],
#                             [1.466,   1.52326,   1.341], # Proline
#                             [1.451,   1.516,   1.32868]], # Glycine
#                            dtype='float32')
#         self.thetas = np.array([[  2.124,   1.941,   2.028],
#                                 [  2.124,   1.96314,   2.028], # Proline
#                                 [  2.124,   1.98793,   2.028]], # Glycine
#                                dtype='float32')
        
        
    def _dihedral_to_point(self, dihedral, name=None):
        ...
        with tf.name_scope(name, 'dihedral_to_point', [dihedral]) as scope:
            dihedral = tf.convert_to_tensor(dihedral, name='dihedral', dtype=tf.float32) # [NUM_STEPS, BATCH_SIZE, NUM_DIHEDRALS]
            ...
            r_cos_theta = tf.constant(self.r * np.cos(np.pi - self.theta), name='r_cos_theta') # [NUM_DIHEDRALS]
            r_sin_theta = tf.constant(self.r * np.sin(np.pi - self.theta), name='r_sin_theta') # [NUM_DIHEDRALS]
            ...
            pt_final = tf.reshape(pt_perm,
                                  [num_steps * self.num_dihedrals, batch_size, self.num_dimensions],# [NUM_STEPS x NUM_DIHEDRALS, BATCH_SIZE, NUM_DIMS]
                                  name=scope) 
            print(pt_final)
            return pt_final
    
    def _ideal_dihedral_to_point(self, dihedral, mask, name=None):
        """ Takes triplets of dihedral angles (phi, psi, omega) and returns 3D points ready for use in
            reconstruction of coordinates. Bond lengths and angles are based on idealized averages.
        Args:
            dihedral: [NUM_STEPS, BATCH_SIZE, NUM_DIHEDRALS]
            mask:     [NUM_STEPS, BATCH_SIZE]
        Returns:
                      [NUM_STEPS x NUM_DIHEDRALS, BATCH_SIZE, NUM_DIMENSIONS]
        """
        ...
        with tf.name_scope(name, 'dihedral_to_point', [dihedral]) as scope:
            ...
            rs_cos_thetas = np.multiply(self.rs, np.cos(np.pi - self.thetas))
            rs_sin_thetas = np.multiply(self.rs, np.sin(np.pi - self.thetas))
            
            print(rs_cos_thetas.shape)
            print(rs_sin_thetas.shape)
            
            masked_rs_sin_thetas = tf.constant(rs_sin_thetas[mask])
            
#             print(rs_cos_thetas[mask])
#             print(rs_sin_thetas[mask])
            
            pt_x = tf.constant(rs_cos_thetas[mask],                        name='pt_x')
            pt_y = tf.multiply(tf.cos(dihedral), masked_rs_sin_thetas,     name='pt_y')
            pt_z = tf.multiply(tf.sin(dihedral), masked_rs_sin_thetas,     name='pt_z')
            print("new")
            print(pt_x)
            print(pt_y)
            print(pt_z)
    ...
    def _point_to_coordinate(self, pt, num_fragments=6, parallel_iterations=4, swap_memory=False, name=None):
        ...
        with tf.name_scope(name, 'point_to_coordinate', [pt]) as scope:
            ...
            init_coords = Triplet(*[tf.reshape(tf.tile(row[np.newaxis], tf.stack([num_fragments * batch_size, 1])), 
                                               [num_fragments, batch_size, self.num_dimensions]) for row in init_mat])
            ...
            pt = tf.reshape(pt, [num_fragments, -1, batch_size, self.num_dimensions]) # [NUM_FRAGS, FRAG_SIZE,  BATCH_SIZE, NUM_DIMENSIONS]
    ...
    def pnerf(self, labels, rev_angle_dict, mask=None):
        """ Takes tokenized labels,
            converts to triplets of dihedral angles (phi, psi, omega) [NUM_STEPS, BATCH_SIZE, NUM_DIHEDRALS]
            returns 3D coordinates
        Args:
            labels: [NUM_STEPS, BATCH_SIZE]
        Returns:
                      [NUM_STEPS x NUM_DIHEDRALS, BATCH_SIZE, NUM_DIMENSIONS]
        """
        rev_angle_dict[0] = (4.5,4.5,np.pi) # Assign padding
        
        # Convert to triplets of dihedral angles
        u,inv = np.unique(labels, return_inverse=True)
        dihedral = np.array([list(rev_angle_dict[x]) for x in u])[inv]
        dihedral = dihedral.reshape([labels.shape[1],labels.shape[0],3], order='F')
        
        # TODO: check reshaping properly

        if mask is None:
            points = self._dihedral_to_point(dihedral)
        else:
            points = self._ideal_dihedral_to_point(dihedral, mask)
        coords = self._point_to_coordinate(points)
